...
echo "\t\t<androidPackage spec=\"tv.superawesome.sdk.publisher:superawesome-admob:8.0.0-alpha3\" />" >> $dependencyFile
echo "\t</androidPackages>" >> $dependencyFile

echo "\t<iosPods>" >> $dependencyFile
echo "\t\t<iosPod name=\"SuperAwesomeAdMob\" version=\"~> 8.0\" bitcodeEnabled=\"true\" minTargetSdk=\"10.0\" />" >> $dependencyFile
echo "\t</iosPods>" >> $dependencyFile
echo "</dependencies>" >> $dependencyFile
